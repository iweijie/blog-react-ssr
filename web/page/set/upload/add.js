...
import { Button, Input, Row, Col, Radio, Form, Modal, Icon, message } from 'antd';
...
class App extends Component {
  ...
  handleOk = () => {
    ...
    let { userInfo, getList } = this.props;
    ...
    validateFields((err, value) => {
      ...
      if (!this.lock) return;
      this.lock = false;
      const close = message.loading('文件正在上传中', 0);
      apis
        .fileUpload(formData)
        .then((data) => {
          const { state, msg } = data;
          if (state === 0) return message.error(msg);
          message.success(msg);
          getList(1);
          this.handleCancel();
        })
          ...
          <Form>
            <Row type="flex" justify="space-between">
              <Col span={24} style={{ position: 'relative' }}>
                ...
                <FormItem>
                  <Button className="set-upload-btn" onClick={this.arouseInput} type="primary" icon="plus">
                    添加文件
                  </Button>
                    ...
                    <ul className="set-upload-selected-file">
                      {fileList.map((v, k) => {
                        return (
                          <li key={k}>
                            {v.name}
                            <Icon onClick={() => this.clearFile(v)} type="close" theme="outlined" />
                          </li>
                        );
                      })}
                    </ul>
