...
class App extends PureComponent {
    ...
    render() {
        ...
        var replyInput = (
            <div style={replyIndex !== null ? { padding: "20px 0" } : null}>
                {replyName ? (
                    <div>
                        回复：<span className="green">{replyName}</span>
                    </div>
                ) : null}
                ...
                <Row>
                    <Button className="mr20" onClick={this.submit}>
                        提交
                    </Button>
                ...
                <li key={v._id || k}>
                    <p>
                        {v.replyName ? (
                            <span>
                                回复{" "}
                                <span className="green">{v.replyName}</span>：
                            </span>
                        ) : null}
                        <span
                            style={
                        ...
                        (!userInfo.isLogin && name === v.name) ? null : (
                            <span
                                className="reply"
                                onClick={() => this.reply(k, v.name, v.userId)}
                            >
                                回复
                            </span>
                        )}
        ...
        return (
            <div style={{ backgroundColor: "#fff", borderRadius: 5 }}>
                <ul className="message-board">{leaveList}</ul>
    ...
    submit = () => {
        ...
        syncArticleLeavesubmit(params).then((result) => {
            if (result.state) {
