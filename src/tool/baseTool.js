...
export const popUpImage = function () {
    ...
    var sh = store.sh = window.innerHeight || document.documentElement.clientheight;
    var sw = store.sw = window.innerWidth || document.documentElement.clientWidth;
        ...
        if (store.sw < store.imgW || store.sh < store.imgH) {
            ...
            store.stie.addEventListener("mousedown", function (event) {
                if (event.button === 0) {
                    ...
                    const w = imgW > sw ? [sw - imgW - 2 * moveSpace, moveSpace] : [-2 * moveSpace, sw - imgW + moveSpace]
                    const h = imgH > sh ? [sh - imgH - 2 * moveSpace, moveSpace] : [2 * moveSpace, sh - imgH + moveSpace]
                    ...
                    const move = (e) => {
                        ...
                        topValue = topValue < h[0] ? h[0] : topValue > h[1] ? h[1] : topValue
                        leftValue = leftValue < w[0] ? w[0] : leftValue > w[1] ? w[1] : leftValue
    ...
    function popUp(src, flag = false) {
        ...
        css(wrap, wrapStyle)
        if (flag) {
            store.sh = sh = window.innerHeight || document.documentElement.clientheight;
